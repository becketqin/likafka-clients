 ...
 * <p>
 * Please notice that if the timestamp type on the broker is set to LogAppendTime, the timestamp passed to the auditor
 * for audit type ATTEMPT and SUCCESS may be different because the broker may override the timestamp specified
 * by the user. To ensure the timestamps are aligned, users should set a timestamp in the ProducerRecord when send the
 * message and also set the broker side timestamp type configuration to CreateTime.
 * <p>
...
public class LiKafkaProducerImpl<K, V> implements LiKafkaProducer<K, V> {
  ...
  public Future<RecordMetadata> send(ProducerRecord<K, V> producerRecord, Callback callback) {
    ...
    try {
      ...
      Object auditInfo = _auditor.getCustomAuditInfo(key, value);
      ...
      try {
        ...
        _auditor.record(auditInfo, topic, partition, timestamp, 1L, 0L, AuditType.ATTEMPT);
        _auditor.record(auditInfo, topic, partition, timestamp, 1L, 0L, AuditType.FAILURE);
        throw new KafkaException(t);
      }
      int sizeInBytes = (serializedKey == null ? 0 : serializedKey.length)
          + (serializedValue == null ? 0 : serializedValue.length);
      // Audit the attempt.
      _auditor.record(auditInfo, topic, null, timestamp, 1L, (long) sizeInBytes, AuditType.ATTEMPT);
      // We wrap the user callback for error logging and auditing purpose.
      Callback errorLoggingCallback =
          new ErrorLoggingCallback<>(messageId, auditInfo, topic, timestamp, sizeInBytes, _auditor, callback);
      ...
      _auditor.record(_auditor.getCustomAuditInfo(producerRecord.key(), producerRecord.value()), producerRecord.topic(),
          producerRecord.partition(), producerRecord.timestamp(), 1L, 0L, AuditType.FAILURE);
      throw new KafkaException(t);
    } finally {
  ...
  private static class ErrorLoggingCallback<K, V> implements Callback {
    ...
    private final Object _auditInfo;
    ...
    public void onCompletion(RecordMetadata recordMetadata, Exception e) {
      if (e != null) {
        LOG.error(String.format("Unable to send event %s with message id %s to kafka topic %s",
            _auditInfo == null ? "[No Custom Info]" : _auditInfo,
            (_messageId != null) ? _messageId.toString().replaceAll("-", "") : "[none]", _topic), e);
        // Audit the failure.
        _auditor.record(_auditInfo, _topic, null, _timestamp, 1L, _serializedSize.longValue(), AuditType.FAILURE);
      } else {
        // Audit the success.
        _auditor.record(_auditInfo, _topic, recordMetadata.partition(), recordMetadata.timestamp(),
            1L, _serializedSize.longValue(), AuditType.SUCCESS);
      }
